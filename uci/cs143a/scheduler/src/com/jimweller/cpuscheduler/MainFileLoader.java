...
//		SchedulingAlgorithm alg;
		...
		if(!(alg instanceof OptionallyPreemptiveSchedulingAlgorithm))
		{
			...
			else if(alg instanceof FCFSSchedulingAlgorithm)
			{
					if (args.length > 3) {
						System.err.println("Too many arguments.");
						System.err.println();
					...
					if (args.length > 2) {
						if (!args[2].toUpperCase().equals("FIRST") && !args[2].toUpperCase().equals("BEST")) {
							System.err.println("Wrong last param. Only FIRST and BEST are allowed");
							System.err.println();
							printAlgorithms();
							System.exit(-5);
						}
						((FCFSSchedulingAlgorithm) alg).setMemoryManagement(args[2].toUpperCase());
				}
			}
			else{
				// 2 params
	...
	private static void printUsage() {
		System.err.println("Usage: java -jar CPUSchedulingSimulator.jar" + " JOBS ALGORITHM");
		System.err.println("       java -jar CPUSchedulingSimulator.jar" + " JOBS ALGORITHM [PREEMPT]");
		System.err.println("       java -jar CPUSchedulingSimulator.jar" + " JOBS RR [QUANTUM]");
	...
	private static void printAlgorithms() {
		...
		for (Map.Entry<String, Class<? extends SchedulingAlgorithm>> entry : algs.entrySet()) {
			try {
				SchedulingAlgorithm alg = entry.getValue().newInstance();
				System.err.printf("    %-16s    %s", entry.getKey(), alg.getName());
				if (alg instanceof OptionallyPreemptiveSchedulingAlgorithm) {
