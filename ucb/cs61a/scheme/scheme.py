...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        # check if formals and vals have same length
...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # eval each operand
        args = operands.map(lambda x: scheme_eval(x, env))
...
class LambdaProcedure(UserDefinedProcedure):
    ...
    def make_call_frame(self, args, env):
        ...
        # return Frame that binds (formals, args) to new Frame
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # case: define value with lambda form
        head = target.first # name
        formals = target.second # params
        body = expressions.second # body

        env.define(head, do_lambda_form(Pair(formals, body), env))
        return head
        # END PROBLEM 10
    else:
...
def do_and_form(expressions, env):
    ...
    # return the result of evaluation of last expression
    while expressions is not nil:
        result = scheme_eval(expressions.first, env)
        # al; result needs to be True
...
def do_or_form(expressions, env):
    ...
    # return the result of evaluation of first satisfied expression
    while expressions is not nil:
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if scheme_truep(test):
            ...
            # print(expressions) => (((< 2 5) 7) (else 8))
            # print(expressions) => (else 8))
...
def make_let_frame(bindings, env):
    ...
    # return type: <scheme.Frame>

    # ((a 1) (b 2) (c 3)) -> Pair('a', Pair('b', Pair('c', nil)))
    #                        Pair(1, Pair(2, Pair(3, nil)))
    formals, vals = nil, nil
    while bindings is not nil:
        expr = bindings.first
        check_form(expr, 2, 2)
        formals =  Pair(expr.first, formals)
        vals = Pair(scheme_eval(expr.second.first, env), vals)
        bindings = bindings.second
    
    # check_formals: check if each formal is distinct
    check_formals(formals)
...
def do_define_macro(expressions, env):
    ...
    # BEGIN Problem 22
    # print(expressions) => ((for formal iterable body) (list (quote map) (list (quote lambda) (list formal) body) iterable))
    # print(expressions.first) => (for formal iterable body)
    # print(expressions.first.second) => (formal iterable body)    
    # print(expressions.second) => ((list (quote map) (list (quote lambda) (list formal) body) iterable))
    # END Problem 22
...
def scheme_call_cc(function, env):
    ...
    # # (lambda (return) (return (quote return-value)) (quote after-return)) 
    # # type: <class 'scheme.LambdaProcedure'>

    # # body.first.second.first => (quote return-value)
    # body = function.body

    # args = Pair(scheme_eval(body.second.first, env),nil) # Pair
    # error_class = ContinuationError(scheme_eval(body.first.second.first, env))
    # procedure = ContinuationProcedure(error_class)
    # check_procedure(procedure)
