    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # Case: BodyguardAnt is already on the place
                if self.ant.container and self.ant.can_contain(insect):
                    self.ant.ant = insect

                # Case: BodyguardAnt needs to be added to the place
                elif insect.can_contain(self.ant): 
                    # since newly added BodyguardAnt will not contain any ant
                    insect.ant = self.ant
                    self.ant = insect
                else:
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        # BEGIN Problem 8
        ant = self.place.ant
        if isinstance(ant, NinjaAnt):
            return False
        return ant is not None
        # END Problem 8
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        if self.place.bees:
            ...
            while place != hive:
                if (len(place.bees) != 0) and dist <= self.max_range and dist >= self.min_range:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 4
        here = self.place
        # use Ant.reduce_armor instead of Insect.reduce_armor
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 8
        """Cut all bees in the current place."""
        self.cut(self.place.bees)
        # END Problem 8
...
class QueenAnt(ThrowerAnt):  # You should change this line
    ...
    def __init__(self):
        ...
        if QueenAnt.num_queens == 0:
            self.true_queen = True # first one is true queen
        QueenAnt.num_queens += 1
        
        
        # END Problem 13

    def boost_ants_behind(self):
        place = self.place.exit
        while place:
            ant = place.ant # ant in the current place

            if ant:
 
                # is it containing an protected_ant?
                if isinstance(ant, BodyguardAnt):
                    protected_ant = ant.ant
                else:
                    protected_ant = None
 
                if ant not in self.buffed_ants: # ant not in doubled
                    ant.damage *= 2
                    self.buffed_ants.append(ant)
                if protected_ant and protected_ant not in self.buffed_ants: # protected_ant exists and not in doubled
                    protected_ant.damage *= 2
                    self.buffed_ants.append(protected_ant)
            # next place
            place = place.exit
...
def make_slow(action):
    ...
    """
    # BEGIN Problem EC
    # if get slowed, takes every two turn to move 1 place
    def slow(colony):
        if colony.time % 2 == 0:
