...
public class JoinOptimizer {
    ...
    public List<LogicalJoinNode> orderJoins(
        ...
        for (int i = 1; i <= this.joins.size(); i++) {
            Set<Set<LogicalJoinNode>> s = enumerateSubsets(joins,i);
            for (Set<LogicalJoinNode> sprime : s) {
                CostCard best = new CostCard();
                best.cost = Double.MAX_VALUE;
                best.card = Integer.MAX_VALUE;
                for (LogicalJoinNode joinToRemove : sprime) {
                    CostCard cc = computeCostAndCardOfSubplan(stats, filterSelectivities, joinToRemove, sprime, best.cost, pc);
                    if (cc != null && cc.cost < best.cost) {
                        best = cc;
                    }
                }
                pc.addPlan(sprime, best.cost, best.card, best.plan);

            }
        }
        List<LogicalJoinNode> res = pc.getOrder(new HashSet<>(this.joins));
        return res != null ? res : this.joins;
    }
